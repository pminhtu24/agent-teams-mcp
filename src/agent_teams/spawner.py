...
from agent_teams import messaging, teams
from agent_teams.models import COLOR_PALETTE, InboxMessage, TeammateMember
from agent_teams.teams import _VALID_NAME_RE
from agent_teams import opencode_client
...
def use_horizontal_split() -> bool:
    """Return True when panes should split horizontally (stacked, new pane below)."""
    return os.environ.get("TMUX_HORIZONTAL_SPLIT") is not None


def _count_panes_in_window() -> int:
    """Count the number of panes in the current tmux window."""
    try:
...
def build_tmux_spawn_args(command: str, name: str) -> list[str]:
    ...
    Layout: Main pane on left, teammates stacked vertically on right.
    - First split: -h (horizontal split) creates right column
    - Subsequent splits: no flag (vertical split) stacks top/bottom on right
    """
    if use_tmux_windows():
        return [
            "tmux",
    ...
    # Old behavior: all panes same direction
    if use_horizontal_split():
        return ["tmux", "split-window", "-dP", "-F", "#{pane_id}", command]

    # Smart layout: first teammate creates right column, subsequent stack vertically
    pane_count = _count_panes_in_window()
    if pane_count == 1:
        # First split: horizontal (-h) → new pane on right
        return ["tmux", "split-window", "-h", "-dP", "-F", "#{pane_id}", command]
    else:
        # Subsequent splits: vertical → stack top/bottom
        return ["tmux", "split-window", "-dP", "-F", "#{pane_id}", command]
